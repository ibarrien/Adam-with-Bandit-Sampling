...
from .samplers import ConditionalStartSampler, VarianceReductionCondition, \
    ...
    LSTMSampler, ConstantVarianceSampler, ConstantTimeSampler, SCSGSampler, BanditSampler
...
class BanditImportanceTraining(_UnbiasedImportanceTraining):
    """
    use the theoretical importance sampling
    Arguments
    ---------
        model: The Keras model to train
        score: {"gnorm", "loss", "full_gnorm"}, the importance metric to use
    ...
    def __init__(self, model, backward_time=2.0, score="gnorm", layer=None):
        ...
        super(BanditImportanceTraining, self).__init__(
            model,
            score,
            layer
        )
