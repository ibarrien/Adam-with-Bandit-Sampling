...
class BanditSampler(BaseSampler):
    """
    This is a sampler based on multi-armed bandit
    """
    ...
    def __init__(self, dataset, reweighting, model, backward_time=2.0,
        ...
        self.p_min = 0.00001/self.N
        self.p_max = 1000.0/self.N
        #self.p_max = 0.1
        self.step_size = 1/float(self.N**3)
        self.L = float('-inf')
        self.l = float('inf')

        self.p = np.array([1/float(self.N)] * self.N)
        super(BanditSampler, self).__init__(dataset, reweighting)

    def sample(self, batch_size):
        idxs = np.random.choice(self.N, batch_size, p = self.p)
        #print(self.p[idxs][0:10])
        x, y = self.dataset.train_data[idxs]
        scores = np.ones(len(idxs))
        w = np.ones((len(idxs), self.reweighting.weight_size))
        #import pdb;pdb.set_trace()

        self._send_messages(idxs, (x, y), w, scores)
    ...
    def update(self, idxs, scores):
        g_j_norm_unnormalized = scores.reshape((-1))
        self.this_idx = idxs
        #w = np.array(self.p)
        self.L = max([self.L] + list(g_j_norm_unnormalized)) 
        self.l = min([self.l] + list(g_j_norm_unnormalized))
        g_j_norm = (g_j_norm_unnormalized - self.l)/float(self.L - self.l)
        w = np.array(self.p)
        for sample_idx in range(len(self.this_idx)):
            #self.L = max(self.L, g_j_norm[sample_idx])
            #print(self.L)
            l_j_hat = -(g_j_norm[sample_idx] ** 2) / self.p[self.this_idx[sample_idx]] ** 2
            l_j = l_j_hat + \
                 (self.L ** 2) / (self.p_min ** 2)
            h_j = l_j / self.p[self.this_idx[sample_idx]]
            h_j_hat = l_j_hat/self.p[self.this_idx[sample_idx]]
            #w[self.this_idx[sample_idx]] *= np.exp(-self.step_size * h_j/self.N)
            w[self.this_idx[sample_idx]] *= np.exp(-self.step_size * h_j_hat)
            #if w[self.this_idx[sample_idx]] == float('inf'):
            #    import pdb;pdb.set_trace()
            if w[self.this_idx[sample_idx]] < self.p_min:
                w[self.this_idx[sample_idx]] = self.p_min
            if w[self.this_idx[sample_idx]] > self.p_max:
                w[self.this_idx[sample_idx]] =  self.p_max
            #import pdb;pdb.set_trace()    
            if 0 in w:
                import pdb;pdb.set_trace()
        #import pdb;pdb.set_trace()
...
class ConstantTimeSampler(BaseSampler):
    ...
    def __init__(self, dataset, reweighting, model, backward_time=2.0,
        ...
        self.bandit_sampler = BanditSampler(dataset, reweighting, model)
    ...
    def sample(self, batch_size):
        ...
        if self.condition.satisfied:
            ...
            # do the hippy shake
            if self.use_bandit is True:
                idxs = np.random.choice(self.N, B, p = self.bandit_sampler.p)
            else:
            ...
            if self.use_bandit is True:
                idxs = np.random.choice(self.N, batch_size, p = self.bandit_sampler.p)
            else:
                idxs = np.random.choice(self.N, batch_size)
            idxs2 = np.arange(len(idxs))
            x, y = self.dataset.train_data[idxs]
    ...
    def update(self, idxs, scores):
        if self.use_bandit is True:
            self.bandit_sampler.update(idxs, scores)
        #import pdb;pdb.set_trace()
        if not self.condition.previously_satisfied:
            self.condition.update(scores)
